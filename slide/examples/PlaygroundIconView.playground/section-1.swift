...
class PlaygroundIconView : NSView {
    ...
    didSet(oldAnimate) {
        if animate != oldAnimate && animate {
            if currentSeesawAngle == 0  {
                // We've never animated, which means we're at the level position. Animate to one end at half the duration.
                XCPCaptureValue("Left Seesaw Position", 0)
                animateSeesawToAngle(maxSeesawAngle, duration: 0.75)
            }
            else {
                // We have previously animated, which means we're at one of the extremes. Animate to the other extreme at the normal duration.
    ...
    override func animationDidStop(_:CAAnimation!, finished:Bool) {
        if finished && animate {
            // We're still animating, so start a new animation to the opposite angle.
...
XCPShowView("Playing Icon View",view)
