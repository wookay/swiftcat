...
- (unsigned long) numberOfBytesToLeavePendingWithData:(NSData*) data length:(NSUInteger) length encoding:(int) encoding;
...
- (BOOL) appendData:(NSData *)data { 
	...
	while( true ) {
		...
		if( contentEnd == -1 ) {
			...
			unsigned long leaveTrailing = [self numberOfBytesToLeavePendingWithData:data length:sizeToPass encoding:currentEncoding];
...
//				HTTPLogVerbose(@"MultipartFormDataParser: Processed %luFMTNSINT" bytes of body", (unsigned long) sizeToPass);
...
- (unsigned long) numberOfBytesToLeavePendingWithData:(NSData*) data length:(NSUInteger) length encoding:(int) encoding {
	...
	unsigned long sizeToLeavePending = 0;
